...
for i in studentsStr.components(separatedBy: "; "){
	...
	if studentsGroups[getPair.group] == nil {
		...
		studentsGroups[getPair.group]?.append(getPair.name) //maybe it would be better to use ! instead of ? but I'm not sure
		}
	}

for group in studentsGroups.keys {
		studentsGroups[group]?.sort() //once again I'm struggling with this dilemma: ! or ? :) So we use ! if we can guaranteed that it's not nill... But can we be sure?
	}
...
class CoordinateAK {
	...
	func middlePoint(_ other: CoordinateAK) -> CoordinateAK? {
		if lat_direction == other.lat_direction && lon_direction == other.lon_direction {
			return try! CoordinateAK(
	...
	static func middlePoint(_ first: CoordinateAK, _ second: CoordinateAK) -> CoordinateAK? {
...
var coord1 = CoordinateAK()
var coord2 = try! CoordinateAK(-6, -6, 6, 6, 6, 6)
...
var coordmid2 = CoordinateAK.middlePoint(coord2, coord1)
